...
class SolverSC1 :
        ...
        for arc in self.graph.getArcList():
            ...
            if not self.graph.isArcReEntrant(arc) :
                ...
                for sourcePhase in xrange(rangeSource):  # source/prod/out normaux
                    if sourcePhase > 0 : predProd += prodList[sourcePhase - 1]

                    predCons = 0
                    Cons = 0
                    for targetPhase in xrange(rangeTarget):  # target/cons/in normaux
                        Cons += consList[targetPhase]
                        if targetPhase > 0 : predCons += consList[targetPhase - 1]

                        W1 = Cons - predProd - step
                        if self.graph.isThresholded():
                            W1 += predCons + thresholdList[targetPhase] - Cons
        ...
        for arc in self.graph.getArcList():
            if not self.graph.isArcReEntrant(arc) :
                ...
                if FM0 % 1 == 0 :
                    ...
                    optBuffer = False
                    self.graph.setInitialMarking(arc, int((int(FM0) + 1) * step))
    ...
    def __addFRow(self, row, arc, step):
        ...
        self.varCoef[self.k] = float(step)
        self.k += 1
