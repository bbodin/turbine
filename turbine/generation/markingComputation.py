from algorithms.cycle import getArcInEmptyCycle, getArcInCycle
from algorithms.scc import tarjan_empty_recursive, strongly_connected_components_with_empty
...
#                           generate preload                           #
########################################################################
def generateInitialMarking(graph, solver = "auto", LPFileName = None):
    """Step 3
    """
    __clearInitialMarking(graph)

    if solver == "SC2" :
        solver = SolverSC2(graph)
    elif solver == "SC1" :
        solver = SolverSC1(graph)
    elif solver ==  "auto" :
